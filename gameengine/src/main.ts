...
function onTileClick(tile: editor.Tile) {//地图网格响应
    ...
    if (tile.source == panel.TXsource[0]) {//赋素材按钮变化
...
function onTXClick(tile: editor.Tile) {//素材网格响应
    ...
    if (x >= 0 && y >= 0) {//若用户已选中地图网格
        ...
        if (tile.walkable == mapTile.walkable) {//地图网格所用素材和用户所选素材可走性一致时
            ...
            if (tile.source == mapTile.source) {//赋素材按钮变化
                panel.sucaibutton.background.color = "#0000FF";
            } else {
                panel.sucaibutton.background.color = "#FF0000";
            }
